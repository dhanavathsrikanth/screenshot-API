...
const screenshotOptionsSchema = {
  ...
  properties: {
    ...
    wait_for_selector: { type: "string", description: "CSS selector to wait for before capture (custom readiness) — agent-browser wait <sel>." },
    wait_for_text: { type: "string", description: "Wait until text appears — agent-browser wait --text." },
    wait_for_url: { type: "string", description: "Wait until URL matches glob — agent-browser wait --url **/pricing." },
    wait_for_condition: { type: "string", description: "JavaScript expression to evaluate for readiness (custom readiness) — agent-browser wait --fn." },
    chat_input: { type: "string", description: "Plain English like 'take screenshot of pricing table' — AI via OpenRouter openrouter/free auto-navigates via nav/footer like agent-browser chat." },
    debug_annotate: { type: "boolean", default: false, description: "Highlight with [1] @e1 — agent-browser screenshot --annotate + highlight." },
    a11y_check: { type: "boolean", default: true, description: "Run axe-core a11y audit (image-alt/color-contrast) via agent-browser a11y before capture." },
    block_popups: { type: "boolean", default: true, description: "Dismiss scroll-triggered popups/dialogs — newsletter modals on scroll." },
    ...
    login_url: { type: "string", format: "uri", description: "Form login URL — agent-browser auth login." },
    username_selector: { type: "string", description: "Username field selector, default input[type=email]." },
    password_selector: { type: "string", description: "Password field selector, default input[type=password]." },
    ...
    paths: {
      "/api/take": {
        get: {
          ...
          description:
            "Renders a website (or HTML/markdown) and returns the image/PDF/video bytes directly. All features via query string (full_page, selector, chat_input, video_seconds, block_popups, debug_annotate, a11y_check, wait_for_text/url, login_url, etc. — like ScreenshotOne). Authenticate with ?access_key=API_KEY (or api_key/key, x-api-key header, Bearer) or HMAC-signed access_key+signature for <img> tags.",
          ...
          parameters: [
            ...
            },
            {
              ...
              name: "wait_for_text",
              schema: { type: "string" },
              description: "Wait until text appears — agent-browser wait --text.",
            },
            {
              in: "query",
              name: "wait_for_url",
              schema: { type: "string" },
              description: "Wait until URL matches — agent-browser wait --url **/pricing.",
            },
            {
              in: "query",
              name: "chat_input",
              schema: { type: "string" },
              description: "Plain English like 'take screenshot of pricing table' — AI via OpenRouter openrouter/free auto-navigates via nav/footer like agent-browser chat.",
            },
            {
              in: "query",
              name: "debug_annotate",
              schema: { type: "boolean" },
              description: "Highlight with [1] @e1 — agent-browser screenshot --annotate.",
            },
            {
              in: "query",
              name: "a11y_check",
              schema: { type: "boolean" },
              description: "Run a11y audit — agent-browser a11y.",
            },
            {
              in: "query",
              name: "block_popups",
              schema: { type: "boolean" },
              description: "Dismiss scroll popups/dialogs.",
            },
            {
              in: "query",
              name: "login_url",
              schema: { type: "string" },
              description: "Form login URL — agent-browser auth login.",
            },
            {
              in: "query",
