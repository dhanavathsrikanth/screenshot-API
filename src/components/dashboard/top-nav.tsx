...
export function DashboardTopNav({ plan }: { plan?: string }) {
  ...
  return (
    <nav className="sticky top-0 z-40 h-14 border-b border-[var(--border)] bg-[var(--card)]/80 backdrop-blur-xl dark:bg-[var(--background)]/80">
      <div className="flex h-full items-center justify-between px-4 lg:px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/"
            className="hidden sm:inline-flex items-center gap-1.5 text-xs font-medium text-[var(--dim)] dark:text-[var(--dim)] hover:text-[var(--ink)] dark:hover:text-[var(--line)] transition-colors"
          >
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
