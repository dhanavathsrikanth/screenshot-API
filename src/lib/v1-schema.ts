...
export const V1ScreenshotRequestSchema = z.object({
  ...
  block_chats: z.coerce.boolean().optional(),
  block_trackers: z.coerce.boolean().optional(),
  block_popups: z.coerce.boolean().optional(),
  block_images: z.coerce.boolean().optional(),
  omit_background: z.coerce.boolean().optional(),
  reduced_motion: z.coerce.boolean().optional(),
  pdf_format: z.enum(["a4", "a3", "a2", "a1", "a0", "legal", "letter", "tabloid"]).optional(),
  pdf_print_background: z.coerce.boolean().optional(),
  ...
  debug_annotate: z.coerce.boolean().optional(),
...
export function buildRenderOptions(input: V1ScreenshotRequest): ScreenshotOptions {
  ...
  return ScreenshotOptionsSchema.parse({
    ...
    wait_for_text: input.wait_for_text,
    wait_for_url: input.wait_for_url,
    ...
    block_chats: input.block_chats ?? true,
    block_trackers: input.block_trackers ?? true,
    block_popups: input.block_popups ?? true,
    block_images: input.block_images ?? false,
    omit_background: input.omit_background ?? false,
    reduced_motion: input.reduced_motion ?? false,
    pdf_format: input.pdf_format,
    pdf_print_background: input.pdf_print_background ?? true,
    ...
    debug_annotate: input.debug_annotate ?? false,
    a11y_check: input.a11y_check ?? true,
